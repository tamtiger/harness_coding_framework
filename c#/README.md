# Lệnh Thực Thi Bắt Buộc & Quy Tắc Khung Dành Cho AI (AI Agent Framework Rules)
...
Các quy tắc tại đây định nghĩa kiến trúc cơ sở (baseline architecture), cách đặt tên (naming conventions), quản lý phụ thuộc (dependency boundaries), quy trình làm việc (agent workflow) và các hàng rào bảo vệ (guardrails) chung. Các quyết định cụ thể theo sản phẩm hoặc nghiệp vụ (database engine, message broker, security policy, observability stack, state machine, adapter contract, v.v.) PHẢI nằm trong `c#/projects/{ProjectName}/`.
...
## 🛑 Các Chỉ Thị Cốt Lõi Không Thể Thương Lượng (Non-Negotiable Core Directives)
1. **Không Ảo Tưởng (No Hallucinations)**: Bạn không được tự suy đoán cấu trúc file. Bạn phải sử dụng công cụ tìm kiếm hoặc xem file nếu không chắc chắn, nhưng lý tưởng nhất là bạn nên tự suy luận ra đường dẫn chính xác dựa trên file `naming-conventions.md` khắt khe của chúng tôi.
...
3. **Không Tự Ý Refactor (No Unprompted Refactoring)**: Không sửa đổi các file ngoài phạm vi tác vụ hiện tại hoặc ngoài thư mục `Feature` cụ thể mà bạn đang tạo, ngoại trừ trường hợp cần thiết để nối (wiring) dependency hoặc phân quyền (permission).
...
4. Áp dụng rule project cho mọi quyết định mang tính sản phẩm/nghiệp vụ; áp dụng rule chung cho kiến trúc và cách tổ chức code.
